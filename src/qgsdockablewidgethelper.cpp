...
std::function<void( Qt::DockWidgetArea, QDockWidget *, const QStringList &, bool )>
    QgsDockableWidgetHelper::sAddTabifiedDockWidgetFunction =
...
QgsDockableWidgetHelper::QgsDockableWidgetHelper( bool isDocked, const QString &windowTitle, QWidget *widget,
                                                  QMainWindow *ownerWindow, Qt::DockWidgetArea defaultDockArea,
                                                  const QStringList &tabifyWith, bool raiseTab,
                                                  const QString &windowGeometrySettingsKey, bool usePersistentWidget )
    : QObject( nullptr ), mWidget( widget ), mDialogGeometry( 0, 0, 0, 0 ),
      mIsDockFloating( defaultDockArea == Qt::DockWidgetArea::NoDockWidgetArea ),
      mDockArea( defaultDockArea == Qt::DockWidgetArea::NoDockWidgetArea ? Qt::DockWidgetArea::RightDockWidgetArea
                                                                         : defaultDockArea ),
      mWindowTitle( windowTitle ), mOwnerWindow( ownerWindow ), mTabifyWith( tabifyWith ), mRaiseTab( raiseTab ),
      mWindowGeometrySettingsKey( windowGeometrySettingsKey ), mUuid( QUuid::createUuid().toString() ),
...
void QgsDockableWidgetHelper::writeXml( QDomElement &viewDom )
{
    ...
    if ( mDock )
    {
        const QList<QDockWidget *> tabSiblings =
            mOwnerWindow ? mOwnerWindow->tabifiedDockWidgets( mDock ) : QList<QDockWidget *>();
...
void QgsDockableWidgetHelper::readXml( const QDomElement &viewDom )
{
...

    {
        ...
        mDockArea = static_cast<Qt::DockWidgetArea>(
            viewDom.attribute( QStringLiteral( "area" ), QString::number( Qt::RightDockWidgetArea ) ).toInt() );
...
QgsDockWidget *QgsDockableWidgetHelper::dockWidget() { return mDock.data(); }

QDialog *QgsDockableWidgetHelper::dialog() { return mDialog.data(); }
...
void QgsDockableWidgetHelper::setupDockWidget( const QStringList &tabSiblings )
{
    ...
    if ( mDockGeometry.isEmpty() && mOwnerWindow )
    {
        ...
        mDockGeometry =
            QRect( static_cast<int>( mOwnerWindow->rect().width() * 0.75 ),
...
QgsNonRejectableDialog::QgsNonRejectableDialog( QWidget *parent, Qt::WindowFlags f ) : QDialog( parent, f ) {}
