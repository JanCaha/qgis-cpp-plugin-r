...
/**
 * This class is responible of displaying a QWidget inside a top level window or a dock widget (only one of the 2 is
 * alive at most). The widget is not owned by this class and its ownership is passed to the owner window before this
 * class's object is deleted or another widget is set using setWidget() function
 *
...
class GUI_EXPORT QgsDockableWidgetHelper : public QObject
{
    ...
    public:
        /**
         * Constructs an object that is responsible of making a docked widget or a window titled \a windowTitle that
         * holds the \a widget The ownership of \a widget is returned to \a ownerWindow once the object is destroyed.
         *
         * If \a usePersistentWidget is TRUE then the \a widget (either as a dock or window) cannot be destroyed and
        ...
        static std::function<void( Qt::DockWidgetArea, QDockWidget *, const QStringList &, bool )>
            sAddTabifiedDockWidgetFunction;
